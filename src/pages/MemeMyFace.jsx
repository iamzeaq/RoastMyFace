...
const MemeMyFace = () => {
    ...
    const handlePackSelect = (pack) => {
        ...
        if (questionItems.length > 0) {
            const randomItem = questionItems[Math.floor(Math.random() * questionItems.length)];
    ...
    const generateRoasts = async () => {
        if (!selectedPack || !selectedPackItem || images.length < selectedPackItem.required_pictures) {
            alert(`Please upload at least ${selectedPackItem.required_pictures} valid image(s).`);
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            alert("Failed to generate roasts. Please try again or select another prompt.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-blue-700 text-white flex flex-col items-center justify-center p-6">
            ...
            <p className="mb-6 text-center max-w-md text-gray-300 font-medium">
                Upload multiple pictures, pick a savage roast pack, and get AI roasts to cry-laugh about 😭
            </p>
                ...
                <div>
                    ...
                    {selectedPack && selectedPackItem && (
                        <div className="flex items-center mb-4">
                            <button
                                onClick={() => {
                                    const questionItems = selectedPack.items.filter(item => item.type === "question");
                                    if (questionItems.length > 1) {
                                        let newItem;
                                        do {
                                            newItem = questionItems[Math.floor(Math.random() * questionItems.length)];
                    ...
                    {images.length > 0 && (
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4 w-full max-w-4xl bg-white p-2 rounded-lg">
                                ...
                                <div className="flex flex-row items-center mb-4 w-full max-w-4xl">
                                    {images.map((img, i) => (
                                        <div key={i} className="flex items-center">
                                            <div
                                                className="overflow-hidden"
                                                style={{ width: `calc(${90 / images.length}vw - ${images.length * 10}px)` }}
                                            >
                                                <img
                                                    src={URL.createObjectURL(img)}
                                                    alt={`Uploaded image ${i + 1} for roast`}
                                                    className="sm:w-full h-30 sm:h-32 object-cover"
                                                />

                                                {imageRoasts.find(roast => roast.imageIndex === i) && (
                                                    <>
                                                        <div className="flex items-center gap-1 pt-1">
                                                            <img src={roaster} alt="RoastMyFace logo" className="rounded-full w-6" />
                                                            <div>
                                                                <h1 className="text-black text-[10px] sm:text-xs">RoastMyFace</h1>
                                                                <p className="text-gray-600 text-[10px] sm:text-xs">@roastmyface</p>
                                                            </div>
                                                        </div>

                                                        <div className="p-1 text-black">
                                                            <p className="font-medium text-xs sm:text-sm">
                                                                {imageRoasts.find(roast => roast.imageIndex === i).roast}
                                                            </p>
                                                        </div>
                                                    </>
                                                )}
                                            ...
                                            {i < images.length - 1 && (
                                                <div className="flex items-center justify-center mx-1 sm:mx-2">
                                                    <span className="text-lg sm:text-xl font-bold text-gray-700">VS</span>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                            ...
                            <div className="text-center mb-6">
                                <p className="mb-2 text-sm">
                                    {images.length}/{selectedPackItem.required_pictures} images uploaded
                                </p>
                                <button
                                    onClick={generateRoasts}
                                    disabled={loading || images.length < selectedPackItem.required_pictures}
                                    className={`px-6 py-2 rounded shadow font-bold ${loading || images.length < selectedPackItem.required_pictures
